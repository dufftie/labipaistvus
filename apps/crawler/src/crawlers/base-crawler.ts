...
const MAX_CONSECUTIVE_FAILURES_DEFAULT = 50;
...
export abstract class BaseCrawler<
  TSubMedia extends string | null = string | null,
> {
  ...
  protected constructor(config: MediaConfig, options: CrawlerOptions = {}) {
    this.config = config;
    this.storage = new ArticleStorage();
    this.MAX_CONSECUTIVE_FAILURES = options.maxConsecutiveFailures ?? MAX_CONSECUTIVE_FAILURES_DEFAULT;
    this.BATCH_SIZE = options.batchSize ?? BATCH_SIZE_DEFAULT;
  }
  ...
  protected async processBatch(
    ...
    const existingIds = await this.storage.getExistingArticleIds(
      this.config.id,
      articleIds,
    );
    ...
    if (skippedRequests.length > 0) {
      ...
      console.log(
        `⊙ Skipped ${ skippedRequests.length } existing articles: ${ skippedIds }`,
      );
    }
    ...
    const crawler = new CheerioCrawler({
      ...
      async requestHandler({$, request, response}) {
        ...
        if (response?.statusCode && response.statusCode >= 400) {
          failureCount++;
          console.log(
            `✗ Article ${ articleId } not found (${ response.statusCode })`,
          );
          return;
        }
        ...
        if (!self.isCorrectDomain(finalUrl)) {
          ...
          console.log(
            `⊘ Article ${ articleId } redirected to different domain: ${ domain }`,
          );
          skipCount++;
          return;
        }
        ...
        if (subMedia === undefined) {
          ...
          const subdomain =
            finalUrl.match(/https?:\/\/([^.]+)\./)?.[1] || "unknown";
          console.log(
            `⊘ Article ${ articleId } on irrelevant subdomain: ${ subdomain }`,
          );
          skipCount++;
          return;
        }
        ...
        if (articleData) {
          try {
            ...
            console.log(
              `✓ Saved article ${ articleId } [${ subMediaLabel }]: ${ articleData.title }`,
            );
          } catch (error) {
  ...
  async run(startFromId?: number, reverse: boolean = false): Promise<void> {
    ...
    console.log(
      `Direction: ${ reverse ? "Reverse (newest to oldest)" : "Forward (oldest to newest)" }`,
    );
    ...
    if (startFromId !== undefined) {
      ...
      const maxArticleId = await this.storage.getHighestArticleId(
        this.config.id,
      );
    ...
    console.log(
      `Last processed batch started at ID: ${ reverse ? currentId + this.BATCH_SIZE : currentId - this.BATCH_SIZE }`,
    );
